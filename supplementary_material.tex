...
Mười kịch bản SC1--SC10 được xây dựng để phủ năm nhóm mối quan tâm:
\begin{enumerate}
\item vận hành bình thường và biến thể địa phương (SC1--SC2);
\item thay đổi quy định, nguồn hoặc phiên bản (SC3, SC6, SC10);
\item gián đoạn hạ tầng/lớp trung ương (SC4, SC7);
\item tổng hợp toàn quốc và quan hệ xuyên địa bàn (SC5, SC8);
\item chuyển giao phạm vi quản lý/ghi giữa địa bàn (SC9).
\end{enumerate}
...
B1 được chấm trước tinh chỉnh. Khi SC9 và SC10 làm lộ khoảng trống, các thay đổi được ghi vào nhật ký tinh chỉnh và tạo B1-R; sau đó cùng bộ SC1--SC10 và cùng rubric được áp dụng lại. Kết quả chi tiết nằm trong \texttt{evaluation\_results.csv}, \texttt{evaluation\_results\_b1r.csv} và \texttt{architecture\_refinement\_log.csv}.
...
\item kết quả B1/B1-R và nhật ký tinh chỉnh;
\item các tệp nguồn sinh hình trong thư mục \texttt{figure\_sources/}.
\end{itemize}

% TODO-WP6: điền định danh phát hành chính thức (DOI/URL/tag/commit/checksum) của gói dữ liệu bổ trợ đã đóng băng chỉ đọc, trước khi nộp bản chính thức.
...
\addlinespace
\textbf{I6. Nền tảng số quốc gia} & Luật 93/2025 khoản 3 Điều 20 (Nhà nước xây dựng, vận hành Nền tảng số quản lý KH,CN\&ĐMST quốc gia); NĐ 268 khoản 3 Điều 48 (UBND cấp tỉnh định kỳ cập nhật dữ liệu vòng đời Giấy chứng nhận DN KH\&CN lên nền tảng quốc gia); NĐ 268 khoản 2 Điều 66 (UBND cấp tỉnh thông báo Bộ trong 15 ngày, áp dụng cả Giấy công nhận DN KNST). & Nền tảng số quốc gia tổng hợp/kết nối/quan sát liên ngành; UBND cấp tỉnh là chủ thể mang nghĩa vụ pháp lý; nút tỉnh đang có gán quyền ghi hiệu lực là SoR nghiệp vụ; nền tảng chuyên ngành đề xuất là phương tiện kỹ thuật hỗ trợ thực hiện nghĩa vụ, không tự mang nghĩa vụ pháp lý. & Hợp đồng dữ liệu cho sự kiện cập nhật/thông báo phải giữ loại giấy (chứng nhận DN KH\&CN phân biệt với công nhận DN KNST), cơ quan quyết định, ngày quyết định/hiệu lực, trạng thái vòng đời, nguồn và phiên bản; hai loại sự kiện (cập nhật vòng đời, thông báo Bộ) mang định danh và đích nhận tách biệt, không gộp. & Trao đổi đi qua cổng tích hợp và hạ tầng chia sẻ/điều phối theo quy định; gửi lại phải lũy đẳng và giữ bằng chứng gửi--nhận; không mở nhánh báo cáo song song thay thế vai trò tổng hợp quốc gia. Giao thức, chu kỳ đồng bộ và kiểu API vẫn là L3. \\
\end{longtable}
\normalsize

Đánh mã: I6 được thêm vào cuối bảng trên (không đổi số I1--I5) để tránh phải
sửa lại mọi tham chiếu "I\emph{n}" hiện có trong repo, đáng chú ý là
\texttt{viewpoint\_registry.csv} hàng V6 đang trỏ ``Bảng 5/I1'' tới đúng
dòng Định danh/xác thực. Ở Bảng~5 (\emph{Các ranh giới liên thông và hệ quả
kiến trúc chính}) của thân bài, dòng ``Nền tảng số quốc gia'' đứng đầu bảng
nhưng mang mã I6 -- thứ tự mã vì vậy không trùng thứ tự dòng; nhãn mã được
hiển thị trực tiếp trong Bảng~5 để người đọc thấy ánh xạ thân bài--bổ trợ.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.96\textwidth]{figure_sources/fig06_c4_deployment_pdist.pdf}
...
\textbf{AD01 -- biên quyền ghi theo thẩm quyền.} Cấu trúc bốn nhóm trường của gán quyền ghi, bất biến single-active-writer, máy trạng thái chuyển giao \textsc{prepared--old-frozen--cutover-confirmed--new-active--completed} và quy tắc hủy/phục hồi tiến/xác minh epoch khi khởi động lại đã được trình bày đầy đủ ở Mục~5.2 của thân bài (Hình~4); phần này chỉ giữ liên kết còn lại với topology: Topology vẫn là VP7; cấu hình dùng trong stress-test là P-DIST. Ngữ nghĩa thiết kế vẫn ở mức L2, không khóa thuật toán consensus, cơ chế khóa hay middleware L3.

\textbf{AD03 -- mô hình nền pháp lý và cấu hình địa phương.} B1-R giữ trạng thái, cổng quyết định và thời hạn L1 trong mô hình nền; địa phương chỉ cấu hình bước nội bộ, vai trò, biểu mẫu và mục tiêu xử lý. Mỗi cấu hình có phiên bản/ngày hiệu lực và qua cổng kiểm tra. Sau SC10, hợp đồng/lược đồ/cấu hình còn phải công bố cửa sổ tương thích và phiên bản tối thiểu còn hỗ trợ; hồ sơ đang xử lý được ghim vào phiên bản quy trình cho đến khi có quy tắc chuyển minh thị.
...
Một bản ghi gán quyền ghi mang bốn nhóm trường tham chiếu (không phải schema bắt buộc của một sản phẩm cụ thể):
\begin{itemize}
...
\end{itemize}
Bất biến single-active-writer, chuỗi trạng thái chuyển giao (\textsc{prepared}$\rightarrow$\textsc{old-frozen}$\rightarrow$\textsc{cutover-confirmed}$\rightarrow$\textsc{new-active}$\rightarrow$\textsc{completed}), quy tắc hủy/phục hồi tiến và quy tắc xác minh epoch khi một nút khởi động lại sau gián đoạn đã được trình bày đầy đủ ở Mục~5.2 của thân bài (Hình~4); phần này chỉ giữ danh mục trường ở trên làm tham chiếu triển khai. Cơ chế khóa, consensus hoặc middleware cụ thể thuộc L3.
...
Phần này giữ các neo kiến trúc và phép kiểm tra chi tiết đã được rút khỏi thân bài, cùng kết quả chấm điểm đầy đủ cho đường cơ sở tập trung B0 (\texttt{evaluation\_results\_b0.csv}). Bảng chính chỉ trình bày phát hiện có ý nghĩa đối với lập luận khoa học; các nội dung dưới đây phục vụ truy vết và chuẩn bị kiểm chứng nguyên mẫu. B0 dùng lại đúng lõi ổn định đã tinh chỉnh (AD01 gán quyền ghi/máy trạng thái, AD03 cấu hình có phiên bản) như B1-R -- chỉ khác lựa chọn topology (VP7: kho tập trung đa tenant thay vì P-DIST) -- nên cột Neo kiến trúc chỉ ghi thêm khi B0 có anchor hoặc điều kiện khác biệt so với B1-R; ô để trống nghĩa là anchor không đổi.

\small
\setlength{\tabcolsep}{2.0pt}
\renewcommand{\arraystretch}{1.02}
\begin{longtable}{@{}p{2.55cm}p{1.35cm}p{1.35cm}p{1.35cm}p{2.95cm}p{4.5cm}@{}}
\toprule
\textbf{Kịch bản} & \textbf{B0} & \textbf{B1} & \textbf{B1-R} & \textbf{Neo kiến trúc} & \textbf{Kiểm chứng hiện thực hóa cần thiết} \\
\midrule
\endfirsthead
\toprule
\textbf{Kịch bản} & \textbf{B0} & \textbf{B1} & \textbf{B1-R} & \textbf{Neo kiến trúc} & \textbf{Kiểm chứng hiện thực hóa cần thiết} \\
...
\bottomrule
\endfoot
SC1. Tỉnh xử lý/quyết định theo thẩm quyền & Trực tiếp & Trực tiếp & Trực tiếp & AD01, AE24, CR2 & Định tuyến theo phạm vi và từ chối ghi ngoài phạm vi đang có hiệu lực; với B0 cần kiểm thử riêng việc cách ly tenant chỉ ở tầng ứng dụng. \\
SC2. Hai tỉnh khác bước nội bộ & Trực tiếp & Trực tiếp & Trực tiếp & AD03, AE26, CR5 & Kiểm tra cấu hình địa phương không nới trạng thái, thẩm quyền hay thời hạn L1. \\
SC3. Quy định thay đổi & Có điều kiện & Có điều kiện & Có điều kiện & AD03, CR6 & Hồi quy cấu hình; phát hành phiên bản mới; xử lý hồ sơ đang chạy và lịch sử. \\
SC4. Chia sẻ trung ương gián đoạn & \textbf{Rủi ro} & Có điều kiện & Có điều kiện & AD04, CR8; B0 thiếu PD1/PD2 (không có kho vận hành tách biệt theo tỉnh) & Gửi lại có kiểm soát, chống xử lý trùng, bảo toàn trạng thái và thời hạn nghiệp vụ; với B0 cần xác minh liệu tỉnh có ghi cục bộ được khi lớp trung tâm gián đoạn hay không. \\
SC5. Ảnh dữ liệu toàn quốc & \textbf{Trực tiếp} & Có điều kiện & Có điều kiện & AE25, CR7; B0 không cần AE25 tách biệt (đọc trực tiếp kho giao dịch) & Đo độ mới, kiểm tra provenance/phiên bản và đối soát với nguồn tạo lập; với B0 cần kiểm thử tranh chấp tài nguyên giữa truy vấn tổng hợp và ghi giao dịch. \\
SC6. Định danh doanh nghiệp đổi ở nguồn quốc gia & Có điều kiện & Có điều kiện & Có điều kiện & AE03, AE15, CR4 & Làm mới/xác minh nguồn và xử lý thay đổi khóa/tham chiếu mà không tạo bản chủ cạnh tranh. \\
SC7. Lớp trung ương không sẵn sàng & \textbf{Rủi ro} & Có điều kiện & Có điều kiện & AD04, V4/V5; B0 thiếu PD1/PD2 & Thử miền lỗi và xác minh bước nào được tiếp tục, dừng hoặc khôi phục theo phụ thuộc ngoài; với B0 cần xác minh còn bước thuần địa phương nào không phụ thuộc lớp trung tâm. \\
SC8. Quan hệ xuyên tỉnh & \textbf{Trực tiếp} & Có điều kiện & Có điều kiện & AD05, CR9; B0 không cần bước hợp nhất liên kho & Hợp nhất quan hệ theo định danh, nguồn, hai đầu quan hệ và thời gian hiệu lực; với B0 cần kiểm thử kiểm soát truy cập theo phạm vi trên cùng một kho. \\
SC9. Chuyển phạm vi quản lý A--B & Có điều kiện & Rủi ro & Có điều kiện & AD01, CR2--CR3; B0 không cần đồng bộ chéo hai kho vật lý & Ghi cạnh tranh, cutover, khởi động lại, gửi lại và forward recovery phải giữ single-active-writer/provenance; với B0 phạm vi kiểm thử hẹp hơn (không có cutover liên kho) nhưng vẫn cần kiểm thử chuyển writer\_epoch/định tuyến nguyên tử. \\
SC10. Nâng lược đồ/quy trình lệch thời điểm & Có điều kiện & Có điều kiện & Có điều kiện & AD03, CR6; ở B0 chỉ còn rủi ro lệch phiên bản cấu hình quy trình, không còn lệch phiên bản lược đồ liên kho & Ma trận tương thích, nâng cấp cuốn chiếu, version pinning và phát lại dữ liệu lịch sử. \\
\end{longtable}
\normalsize

Chi tiết L2 của chuyển quyền ghi theo thời gian được mô tả ở S9.2 và S10.1; chi tiết quản trị tương thích phiên bản nằm ở S9.2. Các phép kiểm tra trên chưa được thực hiện trong nghiên cứu hiện tại và không được diễn giải như kết quả prototype. Lập luận đầy đủ cho từng mức chấm B0 (mechanism, sensitivity\_point, risk\_or\_condition, đối chiếu với B1-R) nằm trong \texttt{evaluation\_results\_b0.csv}; hai kịch bản B0 đạt mức rủi ro kiến trúc (SC4, SC7) phản ánh đánh đổi cố hữu của việc chọn phương án "kho tập trung đa tenant" ở điểm biến thiên VP7 -- đã được ghi nhận sẵn trong \texttt{architecture\_decisions.csv} (AD01, trường \texttt{tradeoffs}) -- không phải một khoảng trống mới của lõi ổn định đòi hỏi sửa AD01--AD05.
